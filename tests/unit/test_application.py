...
def test_use_case_happy_path(mock_generator, guardrail_service):
    use_case = GenerateSpiellinie(generator=mock_generator, guardrail=guardrail_service)
    mock_generator.generate.return_value = "Dies ist eine sehr lange und absolut sichere Spiellinie, die alle Kriterien erfüllt."
    
    result = use_case.execute("Primarschule", "Natur am Rhein")
